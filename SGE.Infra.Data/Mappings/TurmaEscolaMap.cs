...
namespace SGE.Infra.Data.Mappings
{
    public class TurmaEscolaMap : IEntityTypeConfiguration<TurmaEscola>
    {
        public void Configure(EntityTypeBuilder<TurmaEscola> builder)
        {
            ...
            builder.HasOne(te => te.Escola).WithMany(e => e.ListaDeTurmas).HasForeignKey(te => te.IdEscola);
            builder.HasOne(te => te.Turma).WithMany(e => e.ListaDeEscolas).HasForeignKey(te => te.IdTurma);
        }
    }
}
